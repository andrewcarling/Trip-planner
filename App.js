...
function App() {
  ...
  // ---------- Moved out of conditional (fixes your error) ----------
  // Used by Saved view to identify "custom" gear
  const subpackAll = useMemo(() => Object.values(SUBPACKS).flat(), []);
  const isCustomGear = useMemo(
    () => (g) => !BASE_GEAR.includes(g) && !subpackAll.includes(g),
    [subpackAll]
  );
  const customOnly = useMemo(
    () => (saved && saved.gear ? saved.gear.filter(isCustomGear) : []),
    [saved, isCustomGear]
  );
      ...
      <div className="card" style={{ maxWidth: "720px", margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem", marginBottom: 8 }}>
          ...
          >
            ...
            {DEFAULT_DOWNLOADS.concat(userDownloadLib).map((d) => (
